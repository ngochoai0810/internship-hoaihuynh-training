...
# TypedDict documents the exact fields expected from each house record.
...
def load_houses(csv_path: str, limit: int | None = None) -> list[HouseRecord]:
...
def average_price_by_neighborhood(
    groups: dict[str, list[HouseRecord]],
) -> dict[str, float]:
    averages: dict[str, float] = {}
    for name, houses in groups.items():
        prices = [
            house["sale_price"] for house in houses if house["sale_price"] is not None
        ]
        if prices:
            averages[name] = sum(prices) / len(prices)
    return averages


# return top 5 house styles the most popular
def most_common_house_styles(
    houses: list[HouseRecord], top_n: int = 5
) -> list[tuple[str, int]]:
...
def filter_expensive_houses(
    houses: list[HouseRecord], min_price: float
) -> list[HouseRecord]:
    return [
        h
        for h in houses
        if h["sale_price"] is not None and h["sale_price"] >= min_price
    ]
